    ...
    <div class="col-md-12">
      ...
      <div class="box-shadow study-categories__box">
        <div class="box-header">
          <h1 class="study-categories__title"><?= h($courseData->name); ?></h1>
        </div>
        <div class="box-body">
      ...
      <div class="study-categories__list">
        <div class="list__header categories-color__level--<?= $levelColor[$studyData['game_study_categories__level'] - 1] ?>">
          <div class="header__text1">
            <div>
              <span>レベル<?= h($studyData['game_study_categories__level']) ?></span>&nbsp;&nbsp;
              <span>
                <?php
                    $sub_name = str_replace(",", " + ", $studyData['game_study_categories__sub_name'], $cnt);
                    echo h($sub_name);
                ?>
              </span>
            </div>
...
-->
          ...
          <div class="header__text2">
            <h3><?= h($studyData['game_study_categories__name']) ?></h3>
            <div>
              <span>
                <a href="<?= $this->Url->build(['controller'=>'gameStudyCategories', 'action'=>'detail', h($courseData->id), h($studyData['game_study_categories__id'])]) ?>">
                  <button class="study__button box-color-o">概要を見る</button>
                </a>
              </span>
              <span>
              <?php if ($studyData['game_study_categories__is_uploaded'] == 0): ?>
                <a href="<?= $this->Url->build(['controller'=>'gameUploadContents', 'action'=>'add', h($courseData->id), $studyData['game_study_categories__id']]) ?>">
        ...
        <?php if(count($studyData['contents']) > 0): ?>
          ...
          <div class="study-contents__row">
              <div>
                  <h4><span>Study<?= $studyContent['level']?>.</span> <?= $studyContent['name']?></h4>
              </div>
              <div>
                <span>
                  <a href="<?= $this->Url->build(['controller'=>'gameStudyContents', 'action'=>'detail', $studyContent['id']]) ?>">
